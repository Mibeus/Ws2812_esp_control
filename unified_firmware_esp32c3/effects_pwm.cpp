...
void pwmBeginSlot(uint8_t slot) {
  ...
  attachOk[slot] = ledcAttach(pin, PWM_FREQ, PWM_RES);
  Serial.printf("[PWM] slot %u (GPIO%u): ledcAttach %s\n", slot, pin, attachOk[slot] ? "OK" : "ZLYHALO");
  ledcWrite(pin, 0);
}
...
static void renderStatic(uint8_t slot) {
  ...
  ledcWrite(sl.gpio, lastLevel[slot]);
}
...
void pwmUpdateSlot(uint8_t slot) {
  ...
  if (fadingOut[slot]) {
    ...
    if (elapsed >= FADE_MS) {
      ...
      ledcWrite(sl.gpio, (uint8_t)(level * fadeOutFromLevel[slot]));
    }
    return;
  }
